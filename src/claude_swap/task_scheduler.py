...
def _runtime_context() -> dict:
    # The upstream consume/switch paths do not mirror secure-storage overrides.
    # Even a defined empty value differs from unset when choosing a store.
    if "CLAUDE_SECURESTORAGE_CONFIG_DIR" in os.environ:
        raise ClaudeSwitchError(
            "The auto service does not support CLAUDE_SECURESTORAGE_CONFIG_DIR; "
            "no task was changed."
        )
...
def _result(name: str, task: dict | None) -> dict:
    ...
    if task is not None:
        ...
        try:
            ...
            raise ClaudeSwitchError(
                "The installed auto-service runtime context is unreadable."
            ) from exc
    return {
        "task_name": name,
...
def install() -> dict:
    ...
    with _management_lock(name):
        ...
        try:
            ...
            try:
                current = _query_task(name)
                # Old-task removal can fail after disabling or unregistering it.
                # Preserve an unchanged snapshot; otherwise compensate either
                # our old registration or this attempt, never a successor.
                if current != previous:
                    if current is not None:
                        old_description = previous["description"] if previous else None
                        if current["description"] not in (description, old_description):
                            raise ClaudeSwitchError(
                                "The task changed during installation."
                            )
                        _assert_owned(current, sid)
                        _remove_task(name, sid, current["description"])
                    if previous is not None:
