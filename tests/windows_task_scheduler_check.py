...
def cleanup(folder: Path) -> None:
    ...
    if data['sid'] != ts._current_user_sid() or not data['name'].startswith('cswap-auto-ci-'):
        raise AssertionError('Unexpected integration receipt')
    task = ts._query_task(data['name'])
    if task is not None:
        if task['description'] not in data['descriptions'] or task['sid'] != data['sid']:
...
def exercise(folder: Path, python: str, recover: bool) -> None:
    ...
    name = 'cswap-auto-ci-' + os.environ.get('GITHUB_RUN_ID', 'manual') + '-' + uuid.uuid4().hex
    receipt = folder / 'receipt.json'
    receipt.write_text(json.dumps({'name': name, 'sid': sid, 'descriptions': [], 'processes': []}), encoding='utf-8')
    ...
    def cli(flag: str, *, broken=False, expected=0):
        result = subprocess.run(
            [python, '-E', '-P', '-c', _CLI, name, str(receipt), flag, 'yes' if broken else 'no'],
            env=env, cwd=home, capture_output=True, text=True, encoding='utf-8',
            errors='replace', timeout=120, check=False,
        )
        if result.returncode != expected:
            raise AssertionError(f'{flag} exited {result.returncode}: {result.stdout}\n{result.stderr}')
        return result
    ...
    def observe(previous_pid=None, timeout=30):
        ...
        while time.monotonic() < deadline:
            ...
            starts = list(re.finditer(r'Auto-service process started \(pid (\d+)\)', text))
            if starts:
                start = starts[-1]
                pid = int(start.group(1))
                if pid != previous_pid and 'Auto-switch running:' in text[start.end():]:
                    ...
                    if task and task['state'] == 'Running' and identity:
                        ...
                        return identity
            time.sleep(0.25)
        raise AssertionError('No fresh running auto process and CLI banner were observed')
    ...
    try:
        ...
        checks.append('running reinstall: new PID, previous PID gone, custom context and spaced interpreter')

        task = ts._query_task(name)
        ts._start_task(name, sid, task['description'])
        assert process(second['pid'])['created'] == second['created']
        starts_before = output_log.read_text(encoding='utf-8').count('Auto-service process started')
        time.sleep(2)
        assert output_log.read_text(encoding='utf-8').count('Auto-service process started') == starts_before
        checks.append('re-fire: existing task instance retained')

        previous = ts._query_task(name)
        failure = cli('--install-service', broken=True, expected=1)
        ...
        third = observe(second['pid'])
        wait_gone(second)
        checks.append('native start failure: new registration removed, previous running definition restored')
        ...
        if recover:
            ...
            checks.append(f'forced exit: new action observed after {time.monotonic() - started:.1f}s')
            wait_gone(third)
        else:
            fourth = third
        ...
        result = {'checks': checks, 'windows': list(sys.getwindowsversion()),
                  'manual_standard_user': 'NOT_PERFORMED', 'visual_flash_check': 'NOT_PERFORMED'}
