...
def record(tmp_path, *, description=DESCRIPTION, state="Ready", sid=SID):
    ...
    root = ET.fromstring(
        ts._build_task_xml(sid, description, str(tmp_path / "pythonw.exe"), context)
    )
    root.find("t:Settings/t:Enabled", NS).text = (
        "false" if state == "Disabled" else "true"
    )
    return {
        "found": True,
        "state": state,
...
def manager(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ts, "_query_task", lambda name: deepcopy(state["task"]))
    ...
    def register(name, xml):
        ...
        state["task"] = {
            "found": True,
            "state": "Disabled"
            if root.findtext("t:Settings/t:Enabled", namespaces=NS) == "false"
            else "Ready",
            "sid": SID,
            "xml": xml,
    ...
    def start(name, sid, description):
        ...
        root = ET.fromstring(state["task"]["xml"])
        root.find("t:Settings/t:Enabled", NS).text = "true"
        state["task"]["xml"] = ET.tostring(root, encoding="unicode")
...
@pytest.mark.parametrize(
    "failure", [ClaudeSwitchError("start denied"), KeyboardInterrupt()]
)
def test_start_failure_leaves_no_future_trigger(failure, manager, monkeypatch):
    monkeypatch.setattr(ts, "_start_task", Mock(side_effect=failure))
...
def test_query_distinguishes_confirmed_presence_and_absence(
    present, monkeypatch, tmp_path
):
    task = record(tmp_path) if present else None
    monkeypatch.setattr(ts, "_powershell", lambda *args: task or {"found": False})
    assert ts._query_task("probe") == task
...
def test_registration_is_disabled_until_verified_start(tmp_path, monkeypatch):
    task = record(tmp_path, state="Disabled")
...
@pytest.mark.parametrize("phase", ["unchanged", "disabled", "absent"])
@pytest.mark.parametrize("previous_state", ["Running", "Ready", "Disabled"])
def test_failed_old_task_removal_restores_previous_service(
    phase, previous_state, manager, monkeypatch, tmp_path
):
    previous = record(tmp_path, state=previous_state)
    manager["task"] = deepcopy(previous)
    remove = ts._remove_task
    attempted = []

    def fail_once(name, sid, description):
        if not attempted:
            attempted.append(phase)
            if phase == "disabled":
                manager["task"]["state"] = "Disabled"
                root = ET.fromstring(manager["task"]["xml"])
                root.find("t:Settings/t:Enabled", NS).text = "false"
                manager["task"]["xml"] = ET.tostring(root, encoding="unicode")
            elif phase == "absent":
                remove(name, sid, description)
            raise ClaudeSwitchError("old removal failed")
        remove(name, sid, description)

    monkeypatch.setattr(ts, "_remove_task", fail_once)
    with pytest.raises(ClaudeSwitchError, match="old removal failed"):
        ts.install()
    assert attempted == [phase]
    assert manager["task"] == previous
    if phase == "unchanged":
        assert manager["calls"] == []


def test_failed_old_removal_does_not_overwrite_a_successor(
    manager, monkeypatch, tmp_path
):
    manager["task"] = record(tmp_path, state="Running")
    successor = record(tmp_path, description=ts._OWNER + "b" * 32)

    def replace_then_fail(*args):
        manager["task"] = successor
        raise ClaudeSwitchError("old removal failed")

    monkeypatch.setattr(ts, "_remove_task", replace_then_fail)
    with pytest.raises(
        ClaudeSwitchError, match="old removal failed.*recovery also failed"
    ):
        ts.install()
    assert manager["task"] is successor
    assert manager["calls"] == []


@pytest.mark.parametrize("override", ["", "redirected store"])
def test_redirected_secure_storage_refuses_before_any_manager_call(
    override, monkeypatch
):
    monkeypatch.setattr(ts, "_require_windows", lambda: None)
    monkeypatch.setenv("CLAUDE_SECURESTORAGE_CONFIG_DIR", override)
    manager = Mock(side_effect=AssertionError("manager must not be reached"))
    monkeypatch.setattr(ts, "_powershell", manager)
    with pytest.raises(ClaudeSwitchError, match="CLAUDE_SECURESTORAGE_CONFIG_DIR"):
        ts.install()
