"""The private runner restores context before entering the ordinary CLI."""

import base64
import json
import os
import sys
from pathlib import Path
from unittest.mock import Mock

import pytest

from claude_swap import _auto_service as runner


def encoded(value):
    return base64.b64encode(json.dumps(value).encode()).decode()


def context(tmp_path, config=None):
    return {"version": 1, "home": str(tmp_path), "config": config}


@pytest.mark.parametrize(
    "patch",
    [
        {"version": True},
        {"version": 2},
        {"home": "relative"},
        {"config": "relative"},
        {"config": 123},
        {"home": None},
        {"home": "bad\x00path"},
        {"extra": "not permitted"},
    ],
)
def test_invalid_context_is_rejected(tmp_path, patch):
    value = context(tmp_path)
    value.update(patch)
    with pytest.raises(ValueError):
        runner._decode_context(encoded(value))


@pytest.mark.parametrize("value", [None, [], "a string", {}, 1])
def test_context_must_be_complete_object(value):
    with pytest.raises(ValueError):
        runner._decode_context(encoded(value))


@pytest.mark.parametrize("custom", [False, True])
def test_runtime_replaces_stale_scheduler_context(custom, tmp_path, monkeypatch):
    for name in ("HOME", "USERPROFILE", "CLAUDE_CONFIG_DIR"):
        monkeypatch.setenv(name, "stale")
    config = str(tmp_path / "空白 & o'brien") if custom else None
    runner._apply_context(runner._decode_context(encoded(context(tmp_path, config))))
    assert os.environ.get("CLAUDE_CONFIG_DIR") == config
    assert os.environ["HOME"] == str(tmp_path)
    assert os.environ["USERPROFILE"] == str(tmp_path)


@pytest.fixture
def output(tmp_path):
    path = tmp_path / "output.log"
    handler = runner._ServiceLogHandler(
        path, maxBytes=8192, backupCount=3, encoding="utf-8"
    )
    with runner._LogStream(handler) as stream:
        yield stream, handler, path
    handler.close()


def test_output_preserves_fragments_blank_lines_and_flush_is_idempotent(output):
    stream, _, path = output
    assert stream.write("one\n\ntwo") == 8
    stream.flush()
    stream.flush()
    assert path.read_text() == "one\n\ntwo"
    stream.close()
    with pytest.raises(ValueError, match="closed"):
        stream.write("late")


def test_unterminated_output_rotates_with_bounded_records(output):
    stream, handler, path = output
    handler.handle = Mock(wraps=handler.handle)
    text = "x" * 20_000
    assert stream.write(text) == len(text)
    chunks = [call.args[0].getMessage() for call in handler.handle.call_args_list]
    assert "".join(chunks) == text
    assert max(map(len, chunks)) <= 4096
    files = sorted(path.parent.glob("output.log*"), reverse=True)
    assert 1 < len(files) <= 4
    assert all(file.stat().st_size <= 8192 for file in files)
    assert "".join(file.read_text() for file in files) == text


def test_output_failure_does_not_recurse_or_retry_on_close(output, monkeypatch):
    stream, handler, _ = output
    handler.stream.close()
    handler.stream = None
    open_ = Mock(side_effect=OSError("disk unavailable"))
    monkeypatch.setattr(handler, "_open", open_)
    monkeypatch.setattr(sys, "stderr", stream)
    with pytest.raises(OSError, match="disk unavailable"):
        stream.write("message")
    stream.close()
    open_.assert_called_once()


def test_concurrent_writes_are_not_duplicated_by_flush(output):
    from concurrent.futures import ThreadPoolExecutor

    stream, _, path = output

    def write(i):
        stream.write(f"record-{i}\n")
        stream.flush()

    with ThreadPoolExecutor(max_workers=4) as pool:
        list(pool.map(write, range(100)))
    assert sorted(path.read_text().splitlines()) == sorted(
        f"record-{i}" for i in range(100)
    )


@pytest.mark.parametrize("exit_code", [0, 1])
def test_runner_calls_normal_cli_with_context_and_restores_streams(
    exit_code, tmp_path, monkeypatch
):
    from claude_swap import cli, paths

    for name in ("HOME", "USERPROFILE", "CLAUDE_CONFIG_DIR"):
        monkeypatch.setenv(name, "stale")
    monkeypatch.setattr(paths, "get_backup_root", lambda: tmp_path / "backup")
    before_argv, before_stdin = sys.argv, sys.stdin
    before_out, before_err = sys.stdout, sys.stderr
    seen = []
    custom = str(tmp_path / "selected profile")

    def main():
        seen.append((sys.argv[:], os.environ["CLAUDE_CONFIG_DIR"]))
        assert sys.stdin.read() == ""
        assert sys.stdout.encoding == "utf-8"
        print("ordinary CLI output")
        print("ordinary CLI error", file=sys.stderr)
        raise SystemExit(exit_code)

    monkeypatch.setattr(cli, "main", main)
    with pytest.raises(SystemExit) as result:
        runner.run(encoded(context(tmp_path, custom)))
    assert result.value.code == exit_code
    assert seen == [(["cswap", "auto"], custom)]
    assert sys.argv is before_argv and sys.stdin is before_stdin
    assert sys.stdout is before_out and sys.stderr is before_err
    log = (tmp_path / "backup" / "auto-service.log").read_text()
    assert "ordinary CLI output" in log and "ordinary CLI error" in log


def test_default_and_explicit_default_keep_distinct_global_config_paths(
    tmp_path, monkeypatch
):
    from claude_swap import paths

    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    for name in ("HOME", "USERPROFILE", "CLAUDE_CONFIG_DIR"):
        monkeypatch.setenv(name, "stale")
    runner._apply_context(context(tmp_path))
    assert paths.get_global_config_path() == tmp_path / ".claude.json"
    runner._apply_context(context(tmp_path, str(tmp_path / ".claude")))
    assert paths.get_global_config_path() == tmp_path / ".claude" / ".claude.json"


def test_cli_import_errors_reach_the_background_output(tmp_path, monkeypatch):
    import builtins

    from claude_swap import paths

    for name in ("HOME", "USERPROFILE", "CLAUDE_CONFIG_DIR"):
        monkeypatch.setenv(name, "stale")
    monkeypatch.setattr(paths, "get_backup_root", lambda: tmp_path)
    original = builtins.__import__

    def importing(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "claude_swap" and "cli" in fromlist:
            raise ImportError("broken CLI dependency")
        return original(name, globals, locals, fromlist, level)

    monkeypatch.setattr(builtins, "__import__", importing)
    with pytest.raises(ImportError, match="broken CLI dependency"):
        runner.run(encoded(context(tmp_path)))
    assert "broken CLI dependency" in (tmp_path / "auto-service.log").read_text()
